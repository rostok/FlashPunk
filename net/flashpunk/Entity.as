package net.flashpunk
{
	...
	public class Entity extends Tweener
	{
		...
		// rostok, taken from http://www.simppa.fi/blog/extremely-fast-line-algorithm-as3-optimized/
		/**
		 * Checks if this Entity collides with line
		 * @param	x			Virtual x position to place this Entity.
		 * @param	y			Virtual y position to place this Entity.
		 * @param	sX			segment start X position 
		 * @param	sY			segment start Y position.
		 * @param	dX			segment end X position.
		 * @param	dY			segment end Y position.
		 * @return	If the Entity intersects with the position.
		 */
		public function collideSegment(x:Number, y:Number, sX:Number, sY:Number, dX:Number, dY:Number):Boolean
		{
			var shortLen:int = dY - sY;
			var longLen:int = dX - sX;
			...
			if (yLonger)
			{
				...
				{
					if (collidePoint(x, y, sX + i * multDiff, sY + i)) return true;
				}
			}
			else
			{
				...
				{
					if (collidePoint(x, y, sX + i, sY + i * multDiff)) return true;
				}
			}
